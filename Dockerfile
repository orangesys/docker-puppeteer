...
RUN apt-get update && \
  ...
  yarn global add puppeteer@0.10.2 && yarn cache clean && \
  mkdir -p /usr/share/fonts/noto /tmp/noto && \
  wget -P /tmp/noto https://noto-website.storage.googleapis.com/pkgs/NotoSansCJKjp-hinted.zip && \
